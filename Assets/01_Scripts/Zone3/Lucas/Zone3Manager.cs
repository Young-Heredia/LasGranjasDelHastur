...
using LasGranjasDelHastur.Zone3.Lucas;
...
namespace LasGranjasDelHastur.Zone3
{
    ...
    public sealed class Zone3Manager : MonoBehaviour
    {
        ...
        void TryInitialize()
        {
            ...
            if (worldRoot != null)
            {
                Zone3GachaWorldBootstrap.EnsureCelestialGacha(worldRoot);
                Zone3FlautistaEasterEggBootstrap.EnsureFlautistas(worldRoot);
            }
        ...
        void PersistToSaveCache()
        {
            ...
            var outData = sm.CachedData.zone3 ?? new Zone3SaveData();
            outData.valid = true;
            ...
            outData.cells ??= new System.Collections.Generic.List<Zone3CellSaveData>();
            outData.cells.Clear();
            ...
            for (var i = 0; i < cells.Count; i++)
            {
                ...
                if (c == null)
                    continue;
                outData.cells.Add(new Zone3CellSaveData
                {
                    cellId = c.SlotIndex,
